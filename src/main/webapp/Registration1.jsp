...
<div class="form_container">
	<form action="Register1_Register2" method="post">
		...
		<% if(request.getAttribute("err_messages") != null) {	
			if(err_mes[0] != true) { %>
			<div class="alert alert-danger" role="alert">Email address already used !!</div>
		<% } } %>
		...
		<% if(request.getAttribute("err_messages") != null) {	
			if(err_mes[1] != true) { %>
			<div class="alert alert-danger" role="alert"> Please enter a strong password !! </div>
		<% } } %>
		...
		<div class="but_holder">
			...
			<input type="text" style="display : none;" name="emailID" value="<%= session.getAttribute("emailID") %>"/>
